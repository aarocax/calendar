  ...
  Calendar.prototype.update = function () {
    ...
    for (i = 1; i <= lastDay; i++) {
      events = null;
      events = getEventsOfDay(i,this.date.getMonth()+1,this.date.getFullYear(), data);
      if (events) {
        console.log(events);
      };
  ...
  dataWithAjax(new Date(), function(){});
  ...
  window.onload = function() {
    ...
    dataWithAjax(new Date(), function(data){
      console.log(data);
      calendar = new Calendar("calendarContainer", "small", [ "Wednesday", 3 ], [ "#e91e63", "#c2185b", "#ffffff", "#f8bbd0" ]);
      ...
      organizer.setOnClickListener('year-slider',
        function () { showEvents(); console.log("Year back slider clicked"); },
        function () { showEvents(); console.log("Year next slider clicked"); }
      );
    });
  }
  ...
  function dataWithAjax(date, callback) {
    ...
    try {
     $.post( 
        "http://localhost/javascript/calendar/calendar4/response.php",
  ...
  function getEventsOfDay(day, month, year, data) {
      year_events = null;
      month_events = null;
      day_events = null;
      ...
      for (var i = 0; i < data.years.length; i++) {
        if (data.years[i].int == year) {
          year_events = data.years[i];
          break;
        }
      }
      if ( year_events == null) {
      ...
      // buscamos el mes
      for (var i = 0; i < year_events.months.length; i++) {
        if (year_events.months[i].int == month) {
          month_events = year_events.months[i];
          break;
        }
      }
      ...
      // buscamos el día
      for (var i = 0; i < month_events.days.length; i++) {
        if (month_events.days[i].int == day) {
          day_events = month_events.days[i];
          break;
        }
      }
      if (day_events !== null) {
